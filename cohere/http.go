...
func (p *cohereProvider) requestURL(path string) (string, error) {
...
func (p *cohereProvider) headersForCall(perCall http.Header) http.Header {
	h := cloneHeader(p.headers)
...
func (p *cohereProvider) executeJSON(ctx context.Context, path string, body []byte, perCall http.Header) (*apiResponse, error) {
	if p.err != nil {
		return nil, p.err
	}
...
func (p *cohereProvider) executeStream(ctx context.Context, path string, body []byte, perCall http.Header) (*httpStreamResponse, error) {
	if p.err != nil {
		return nil, p.err
	}
...
func (p *cohereProvider) executeBuffered(ctx context.Context, method, path string, body []byte, perCall http.Header) (*apiResponse, error) {
	requestURL, err := p.requestURL(path)
	if err != nil {
		return nil, err
	}
...
func (p *cohereProvider) readResponse(resp *http.Response) (*apiResponse, error) {
...
func (p *cohereProvider) readAPIError(resp *http.Response) *APICallError {
	defer resp.Body.Close()
	body, truncated, err := readLimited(resp.Body, p.maxErrorResponseBytes)
	if err != nil {
		return &APICallError{Message: err.Error(), Status: resp.StatusCode, Headers: cloneHeader(resp.Header), RequestID: resp.Header.Get("x-request-id"), Cause: err}
	}
...
func (p *cohereProvider) waitBeforeRetry(ctx context.Context, attempt int, resp *http.Response, err error) error {
...
func (p *cohereProvider) retryDelay(attempt int, resp *http.Response) time.Duration {
	if resp != nil {
